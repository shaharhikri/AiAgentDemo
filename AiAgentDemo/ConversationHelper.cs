...
namespace AiAgentDemo
{
    ...
    {
        public static bool IsBlocked(Classification classification, string prompt, out string blockingMessage)
        {
        ...
        public static string GetBlockingMessage(ConversationLanguage lang, Classification classification)
        {
        ...
        public static ConversationLanguage DetectLanguage(string prompt)
        {
            foreach (var c in prompt)
            {
                if (c >= 0x0590 && c <= 0x05FF) return ConversationLanguage.Hebrew; // Hebrew
                if (c >= 0x0600 && c <= 0x06FF) return ConversationLanguage.Arabic; // Arabic
                if (c >= 0x0400 && c <= 0x04FF) return ConversationLanguage.Russian; // Cyrillic (Russian)
            }
