...
internal class RavenSetup(IDocumentStore store)
{
    public const string OpenAiApiKeyEnvironmentVariable = "RAVEN_AI_INTEGRATION_OPENAI_API_KEY";
    private const string OpenAiEndpoint = "https://api.openai.com/";
    private const string CoachModel = "gpt-5-mini";
    private const string GuardModel = "gpt-4o-mini";

    private static readonly HashSet<string> Languages = Enum.GetNames<ConversationLanguage>().ToHashSet();
    private static readonly HashSet<string> Classifications = Enum.GetNames<Classification>().ToHashSet();

    public async Task EnsureDatabaseExistsAsync(CancellationToken cancellationToken = default)
    {
        var record = await store.Maintenance.Server.SendAsync(new GetDatabaseRecordOperation(store.Database), cancellationToken);
        if (record is not null)
            return;

        await store.Maintenance.Server.SendAsync(new CreateDatabaseOperation(new DatabaseRecord(store.Database)), cancellationToken);
    }

    public async Task SeedSampleDataAsync(CancellationToken cancellationToken = default)
    {
        using var session = store.OpenAsyncSession();
        var hasAnyUsers = await session.Query<User>().AnyAsync(token: cancellationToken);
        if (hasAnyUsers)
            return;
        ...
        for (var i = 1; i <= 10; i++)
        {
            ...
            if (i == 1)
                user.TelegramUserId = 1445014427;
            ...
            for (var h = 0; h < 24; h++)
            {
                ...
                session.TimeSeriesFor(user.Id, "HeartRate").Append(timestamp, new[] { rand.NextDouble() * 80 + 60 });
                session.TimeSeriesFor(user.Id, "Steps").Append(timestamp, new[] { rand.NextDouble() * 300 });
                session.TimeSeriesFor(user.Id, "DistanceMeters").Append(timestamp, new[] { rand.NextDouble() * 200 });
                session.TimeSeriesFor(user.Id, "CaloriesBurned").Append(timestamp, new[] { rand.NextDouble() * 20 });
            }
        }
    ...
    public async Task<AgentIdentifiers> EnsureAgentsAsync(CancellationToken cancellationToken = default)
    {
        var apiKey = Environment.GetEnvironmentVariable(OpenAiApiKeyEnvironmentVariable);
        if (string.IsNullOrEmpty(apiKey))
            throw new InvalidOperationException($"Please set the {OpenAiApiKeyEnvironmentVariable} environment variable with your OpenAI API key.");

        var guardConnection = CreateConnectionString("open-ai-guards", apiKey, GuardModel, temperature: 0);
        var coachConnection = CreateConnectionString("open-ai-agent", apiKey, CoachModel, reasoningEffort: OpenAiReasoningEffort.Low, seed: 42);

        store.Maintenance.Send(new PutConnectionStringOperation<AiConnectionString>(guardConnection));
        store.Maintenance.Send(new PutConnectionStringOperation<AiConnectionString>(coachConnection));

        var preGuardId = (await store.AI.CreateAgentAsync(
            new AiAgentConfiguration("pre-guard", guardConnection.Name, PreGuardPrompt()), GuardReply.Instance, cancellationToken)).Identifier;

        var postGuardId = (await store.AI.CreateAgentAsync(
            new AiAgentConfiguration("post-guard", guardConnection.Name, PostGuardPrompt()), GuardReply.Instance, cancellationToken)).Identifier;

        var coachAgentId = await CreateCoachAgentAsync(coachConnection.Name, cancellationToken);
        return new AgentIdentifiers(preGuardId, postGuardId, coachAgentId);
    }

    private static AiConnectionString CreateConnectionString(string name, string apiKey, string model, double? temperature = null, OpenAiReasoningEffort? reasoningEffort = null, int? seed = null)
    {
        var settings = reasoningEffort.HasValue || seed.HasValue
            ? new OpenAiSettings(apiKey, OpenAiEndpoint, model, reasoningEffort: reasoningEffort, seed: seed)
            : new OpenAiSettings(apiKey, OpenAiEndpoint, model);

        if (temperature.HasValue)
            settings.Temperature = temperature.Value;

        return new AiConnectionString { Name = name, Identifier = name, ModelType = AiModelType.Chat, OpenAiSettings = settings };
    }

    private async Task<string> CreateCoachAgentAsync(string connectionStringName, CancellationToken cancellationToken)
    {
        var sampleParams = JsonConvert.SerializeObject(new { from = DateTime.UtcNow.AddDays(-2), to = DateTime.UtcNow });

        var agent = new AiAgentConfiguration(name: "Fitness Coach", connectionStringName: connectionStringName, systemPrompt: CoachPrompt())
        {
            ...
            Queries =
            {
                new AiAgentToolQuery
                {
                    ...
                    Query = "from Users as u where id() = $userId select u",
                ...
                new AiAgentToolQuery
                {
                    ...
                    Query = "from Users where id() = $userId select timeseries(from Steps between $from and $to)",
                    ParametersSampleObject = sampleParams
                },
                new AiAgentToolQuery
                {
                    Name = "get-heart-rate",
                    Description = "Get heart rate samples for a given time range.",
                    Query = "from Users where id() = $userId select timeseries(from HeartRate between $from and $to)",
                    ParametersSampleObject = sampleParams
                },
                new AiAgentToolQuery
                {
                    ...
                    Query = "from Users where id() = $userId select timeseries(from CaloriesBurned between $from and $to)",
                    ParametersSampleObject = sampleParams
                },
                new AiAgentToolQuery
                {
                    ...
                    Query = "from Users where id() = $userId select timeseries(from DistanceMeters between $from and $to)",
                    ParametersSampleObject = sampleParams
                },
                new AiAgentToolQuery
                {
                    ...
                    Query = "from Users where id() = $userId select timeseries(from Steps between $from and $to group by '1 day' select sum())",
                    ParametersSampleObject = sampleParams
                }
            }
        };

        agent.Parameters.Add(new AiAgentParameter("userId", "The current user identifier", sendToModel: false));
        return (await store.AI.CreateAgentAsync(agent, CoachResponse.Instance, cancellationToken)).Identifier;
    }

    private static string PreGuardPrompt()
    {
        var classifications = new HashSet<string>(Classifications);
        ...
        return $"""
               ...
               The input may be in {string.Join("/", Languages)}
    ...
    private static string PostGuardPrompt()
    {
        var classifications = new HashSet<string>(Classifications);
        classifications.Remove(Classification.Unknown.ToString());
        return $"""
    ...
    private static string CoachPrompt() => $"""
               ...
               User may communicate in one of those languages: {string.Join("/", Languages)}.
